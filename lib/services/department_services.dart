...
class DepartmentServices {
 ...
 static Future<void> createDepartment({required String organizationId, required RxString name, required RxString description, required RxBool isLoading}) async {
   final Dio dio = Dio();
    ...
    print('http://192.168.1.8:3000/organizations/$organizationId/departments/create');
    try {
      final response = await dio.post(
        'http://192.168.1.8:3000/api/organizations/$organizationId/departments/create',
        data: {
          "name": name.value,
 ...
 static Future<void> updateDepartmentData({required String deptId,required String name,required String description})async{
    ...
    try {
      final response = await Dio().put(
          'http://192.168.1.8:3000/api/organizations/$orgId/departments/$deptId',
          options: Options(
            headers: {
              'authorization':
 ...
 static Future<void> deleteDepartmentData({required String deptId})async{
    ...
    try {
      final response = await Dio().delete(
        'http://192.168.1.8:3000/api/organizations/$orgId/departments/$deptId/delete',
        options: Options(
          headers: {
            'authorization':
            'Bearer ${CacheHelper().getData(key: 'accessToken')}',
