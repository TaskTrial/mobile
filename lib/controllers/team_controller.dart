...
class TeamController extends GetxController{
  ...
  Future<void> createTeam(String organizationId) async {
    ...
    print('http://192.168.1.8:3000/api/organization/$organizationId/team');
    try {
      final response = await _dio.post(
        'http://192.168.1.8:3000/api/organization/$organizationId/team',
        data: {
          "name": name.value,
        },
        options: Options(
  ...
  Future<void> updateTeamData({required String teamId,required String name,required String description})async{
    ...
    try {
      final response = await Dio().put(
          'http://192.168.1.8:3000/api/organization/$orgId/team/$teamId',
          options: Options(
            headers: {
              'authorization':
   ...
   Future<void> updateTeamAvatar({required teamId}) async {
    ...
    try {
      ...
      var response = await Dio().post(
        'http://192.168.1.8:3000/api/organization/$organizationId/team/$teamId/avatar/upload',
        data: formData,
        options: Options(
          headers: {
